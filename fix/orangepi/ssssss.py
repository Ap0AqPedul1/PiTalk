...
class ClientApp(QtWidgets.QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("TCP Client PyQt Auto Connect + Client List Refresh")
        self.setGeometry(100, 100, 600, 400)
        self.sock = None

        # Nama client otomatis
        self.DEFAULT_CLIENT_NAME = f"Client{random.randint(1000,9999)}"
        ...
        # Auto-connect saat aplikasi dijalankan
        self.connect_server()
    ...
    def connect_server(self):
        ...
        try:
            self.sock.connect(('127.0.0.1', 6006))  # Ganti IP & port jika perlu
            ...
            self.reconnect_timer.stop()  # Stop reconnect jika berhasil
            self.list_timer.start(5000)  # Update list tiap 5 detik
        except Exception as e:
            self.status_label.setText(f"Status: Gagal konek: {e}, mencoba lagi...")
            self.sock = None
            if not self.reconnect_timer.isActive():
                self.reconnect_timer.start(3000)  # Coba reconnect tiap 3 detik
